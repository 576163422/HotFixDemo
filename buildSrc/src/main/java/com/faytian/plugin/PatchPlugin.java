...
//        boolean debugOn = hotfix.isDebugOn();//此时获取不到值，因为apply方法是在引入插件时执行，此时还没有执行到插件拓展（hotfix）里
    ...
    private void processJar(String applicationName, File file, Map<String, String> md5Map, PatchGenerator patchGenerator) {
        try {
            applicationName = applicationName.replaceAll(Matcher.quoteReplacement(File.separator), "/");
//            File bakJar = new File(file.getParent(), file.getName() + ".bak");//todo
//            JarOutputStream jarOutputStream = new JarOutputStream(new FileOutputStream(bakJar));//todo
            //解压jar文件
...
//                    jarOutputStream.write(byteCode);//todo
                } else {
                    //无法热修复的class
                    //输出到临时文件
//                    jarOutputStream.write(IOUtils.toByteArray(is));//todo
                }
                is.close();
//                jarOutputStream.closeEntry();//todo
            }
//            jarOutputStream.close();//todo
            jarFile.close();
//            file.delete();//todo
//            bakJar.renameTo(file);//todo
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
